<?php
	...
	namespace MehrItMiddlewareChainTest\Unit\Middleware;


	use ArrayIterator;
	use EmptyIterator;
	use MehrIt\EliMiddlewareChain\Middleware\ChainMiddleware;
	...
	class ChainMiddlewareTest extends TestCase
	{
		...
		public function testWithoutMiddleware_iterator() {
			...
			$chain = new ChainMiddleware(new EmptyIterator());
		...
		public function testOneMiddleware() {
			...
			$chain = new ChainMiddleware([$m1]);


			$this->assertSame($response1, $chain->process($request1, $handler));
		...
		public function testOneMiddleware_iterator() {
			...
			$chain = new ChainMiddleware(new ArrayIterator([$m1]));
		...
		public function testOneMiddleware_resolverFunction() {
			...
			$chain = new ChainMiddleware(new ArrayIterator([function () use ($m1) {
				return $m1;
			}
			]));
		...
		public function testMultipleMiddleware() {
			...
			$chain = new ChainMiddleware([$m1, $m2]);
		...
		public function testMultipleMiddleware_iterator() {
			...
			$chain = new ChainMiddleware(new ArrayIterator([$m1, $m2]));
		...
		public function testMultipleMiddleware_resolverFunction() {
			...
			$chain = new ChainMiddleware(new ArrayIterator([
				function () use ($m1) {
					return $m1;
				},
				function () use ($m2) {
